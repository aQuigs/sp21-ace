...
/**
 * Fails unless the text sits on one line inside its node. Judged by where the line ends, since a text given more room than it
 * needs is laid out at the full width and reads as overflowing its node, and Compose pads a letter-spaced text's intrinsic width
 * by half a pixel it never draws. A line taller than its node, as a sp line height in a small square makes it, sets the letters
 * off centre or out of view.
 */
fun TextLayoutResult.assertFitsOnOneLine() {
    val ends = getLineRight(0)
    val tall = multiParagraph.height
    val cutShort = multiParagraph.didExceedMaxLines

    assertTrue(
        "${layoutInput.text} ends at ${ends}px of ${size.width}px and is ${tall}px of ${size.height}px tall on $lineCount lines, cut short: $cutShort",
        lineCount == 1 && !cutShort && ends <= size.width && tall <= size.height,
    )
}
